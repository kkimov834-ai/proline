...
export default function Home() {
  ...
  function stopMobileAutoScroll() {
    if (mobileAutoScrollFrame.current !== null) window.clearInterval(mobileAutoScrollFrame.current);
    mobileAutoScrollFrame.current = null;
  }
  function scheduleMobileAutoScroll() {
    if (mobileAutoScrollFrame.current !== null) return;
    mobileAutoScrollFrame.current = window.setInterval(() => {
      const board = boardRef.current;
      const pendingDrag = mobileDragRef.current;
      if (!board || !pendingDrag.active || !pendingDrag.order) return stopMobileAutoScroll();
      const rightEdge = window.innerWidth - 50;
      const direction = pendingDrag.lastX >= rightEdge ? 1 : pendingDrag.lastX <= 50 ? -1 : 0;
      if (!direction) return;
      board.scrollLeft = clampScrollLeft(board.scrollLeft + direction * 10, board.scrollWidth, board.clientWidth);
