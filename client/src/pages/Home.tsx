...
import { autoScrollViewportDelta, clampScrollLeft, edgeScrollSpeed } from "@shared/mobileBoard";
...
export default function Home() {
  ...
  function scheduleMobileAutoScroll() {
    ...
    const tick = () => {
      ...
      const edge = Math.min(50, Math.max(30, bounds.width * 0.12));
      const delta = edgeScrollSpeed(pendingDrag.lastX, bounds.left, bounds.right, edge, 18);
      if (!delta) return;
  ...
  function handlePointerDragMove(event: React.PointerEvent<HTMLElement>) {
    const board = boardRef.current;
    const pendingDrag = mobileDragRef.current;
    if (event.pointerType !== "touch" || !board || !pendingDrag.order || pendingDrag.axis === "vertical") return;
  ...
  async function login(e: React.FormEvent) { e.preventDefault(); const normalizedEmail = email.trim().toLowerCase(); const normalizedCode = code.replace(/\D/g, "").slice(0, 6); try { const result = await loginMutation.mutateAsync({ email: normalizedEmail, code: normalizedCode }); setLoggingOut(false); setUser({ email: result.email, role: result.role as Role, label: result.label, column: result.column as ColumnId | undefined, userId: result.userId }); try { if (result.token) sessionStorage.setItem("proline_session_token", result.token); } catch { /* storage unavailable */ } try { const AudioContextClass = window.AudioContext || (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext; if (AudioContextClass) { audioRef.current = audioRef.current || new AudioContextClass(); await audioRef.current.resume(); audioUnlockedRef.current = true; } } catch { /* audio permission unavailable */ } setLoginError(""); toast.success(`${result.label} kimi daxil oldunuz`); } catch { setLoginError("Email və ya giriş kodu yanlışdır."); } }
