...
import { autoScrollDelta, clampScrollLeft } from "@shared/mobileBoard";
...
export default function Home() {
  ...
  const commentsQuery = trpc.board.comments.useQuery(selected ? { orderId: Number(selected.id) } : { orderId: 0 }, { enabled: !!selected, refetchInterval: selected ? 5000 : false });
  ...
  const visibleOrders = useMemo(() => orders.filter((o) => { const matchesText = `${o.title} ${o.id} ${o.description}`.toLowerCase().includes(query.toLowerCase()); const matchesStatus = statusFilter === "all" || o.column === statusFilter; const created = new Date(o.createdAt); const matchesFrom = !dateFrom || created >= new Date(`${dateFrom}T00:00:00`); const matchesTo = !dateTo || created <= new Date(`${dateTo}T23:59:59`); return matchesText && matchesStatus && matchesFrom && matchesTo; }), [orders, query, statusFilter, dateFrom, dateTo]);
  ...
  async function logout() { if (loggingOut) return; setLoggingOut(true); setUser(null); setOrders([]); setNotices([]); setStaff([]); setSelected(null); setCreating(false); setSettingsOpen(false); setCommentBody(""); try { await logoutMutation.mutateAsync(); await Promise.all([trpcUtils.board.session.reset(), trpcUtils.board.list.reset(), trpcUtils.board.audit.reset(), trpcUtils.board.export.reset(), trpcUtils.board.notificationPreference.reset(), trpcUtils.board.comments.reset()]); setLoggingOut(false); } catch { toast.error("Çıxış tamamlanmadı. Yenidən cəhd edin."); } }
  ...
  async function downloadExport(format: "csv" | "xls", requestedFields?: string[]) {
    try {
      ...
      const matrix = rows.map((row) => fields.map((field) => valueFor(row as Record<string, unknown>, field)));
      if (format === "csv") downloadFile([fields.map(escape).join(","), ...matrix.map((line) => line.map(escape).join(","))].join("\\n"), `proline-${excelTemplate}-${Date.now()}.csv`, "text/csv;charset=utf-8");
        ...
        const table = `<table><caption>${escapeHtml(title)}</caption><thead><tr>${fields.map((field) => `<th>${escapeHtml(field)}</th>`).join("")}</tr></thead><tbody>${matrix.map((line) => `<tr>${line.map((value) => `<td>${escapeHtml(value)}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
    ...
    } catch { toast.error("Excel faylı hazırlana bilmədi."); }
  }
  async function login(e: React.FormEvent) { e.preventDefault(); const normalizedEmail = email.trim().toLowerCase(); const normalizedCode = code.replace(/\D/g, "").slice(0, 6); try { const result = await loginMutation.mutateAsync({ email: normalizedEmail, code: normalizedCode }); setLoggingOut(false); setUser({ email: result.email, role: result.role as Role, label: result.label, column: result.column as ColumnId | undefined, userId: result.userId }); try { const AudioContextClass = window.AudioContext || (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext; if (AudioContextClass) { audioRef.current = audioRef.current || new AudioContextClass(); await audioRef.current.resume(); } } catch { /* audio permission unavailable */ } setLoginError(""); toast.success(`${result.label} kimi daxil oldunuz`); } catch { setLoginError("Email və ya giriş kodu yanlışdır."); } }
  ...
  return <div className="min-h-screen bg-[#08121B] text-[#E9EEF2]" style={{ backgroundColor: activeTheme.surface, backgroundImage: `linear-gradient(rgba(8,18,27,.94), rgba(8,18,27,.97)), radial-gradient(circle at 85% 0%, ${activeTheme.glow}, transparent 34%), url('/manus-storage/proline-grid_53b97e55.png')`, backgroundSize: "cover", backgroundAttachment: "fixed", "--proline-accent": activeTheme.accent, "--proline-accent-hover": activeTheme.accentHover, "--proline-glow": activeTheme.glow, "--proline-surface": activeTheme.surface } as React.CSSProperties}>
    ...
    {selected && !creating && <OrderModal order={selected} user={user} auditLogs={orderAuditQuery.data || []} comments={commentsQuery.data || []} commentBody={commentBody} setCommentBody={setCommentBody} onSubmitComment={submitComment} onClose={() => { setSelected(null); setCommentBody(""); }} onEdit={() => openEdit(selected)} />}
    ...
    {settingsOpen && <SettingsPanel templateId={excelTemplate} setTemplateId={setExcelTemplate} themeId={themeId} setThemeId={setThemeId} columnLayouts={columnLayouts} setColumnLayouts={setColumnLayouts} onExport={(format, fields) => { setSettingsOpen(false); void downloadExport(format, fields); }} onClose={() => setSettingsOpen(false)} />}
  </div>;
}
...
function OrderCard({ order, canDrag, onClick, onDragStart, onDragEnd, onTouchStart, onTouchMove, onTouchEnd }: { order: Order; canDrag: boolean; onClick: () => void; onDragStart: () => void; onDragEnd: () => void; onTouchStart?: (event: React.TouchEvent) => void; onTouchMove?: (event: React.TouchEvent) => void; onTouchEnd?: (event: React.TouchEvent) => void }) { const [p, color] = priorityMeta(order.priority); const [now, setNow] = useState(Date.now()); const [expanded, setExpanded] = useState(false); useEffect(() => { const id = window.setInterval(() => setNow(Date.now()), 60000); return () => window.clearInterval(id); }, []); const pending = order.pendingTo ? columns.find((c) => c.id === order.pendingTo) : null; const hasLongDescription = (order.description || "").length > 150; return <article draggable={canDrag && !order.pendingTo} onDragStart={onDragStart} onDragEnd={onDragEnd} onTouchStart={canDrag && !order.pendingTo ? onTouchStart : undefined} onTouchMove={canDrag && !order.pendingTo ? onTouchMove : undefined} onTouchEnd={canDrag && !order.pendingTo ? onTouchEnd : undefined} onClick={onClick} style={{ touchAction: canDrag && !order.pendingTo ? "none" : "pan-y" }} className={`group min-w-0 overflow-hidden rounded-lg border bg-[#122230] p-3.5 transition-all duration-200 cursor-pointer shadow-[0_4px_18px_rgba(0,0,0,.12)] ${pending ? "border-[#D7B45A]/45" : "border-white/[.08] hover:border-[#D78A4A] proline-theme-border/50 hover:bg-[#162A39]"}`}><div className="flex items-start justify-between gap-2"><div className="flex items-center gap-2 min-w-0">{canDrag && <GripVertical size={14} className="text-[#4F626F] group-hover:text-[#D78A4A] proline-theme-accent shrink-0 transition" />}<span className="font-display text-[10px] font-bold tracking-[.12em] text-[#80909C]">{order.id}</span></div><span className="text-[9px] font-bold uppercase tracking-[.1em] flex items-center gap-1" style={{ color }}><span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: color }} />{p}</span></div><h3 className="min-w-0 break-words font-display text-sm font-semibold text-[#E8EEF2] mt-3 leading-5">{order.title}</h3>{order.image && <img src={order.image} alt="Sifariş şəkli" className="mt-3 h-32 w-full rounded-md object-cover border border-white/10" />}{order.description && <div className="mt-1.5"><p className={`max-w-full break-words whitespace-pre-wrap text-[11px] leading-4 text-[#80909C] ${expanded || !hasLongDescription ? "" : "line-clamp-2 overflow-hidden"}`}>{order.description}</p>{hasLongDescription && <button type="button" onClick={(event) => { event.stopPropagation(); setExpanded((value) => !value); }} className="mt-1 inline-flex min-h-7 items-center rounded-md px-1.5 text-[10px] font-semibold proline-theme-accent hover:bg-white/[.06]">{expanded ? "Qısalt" : "Tam göstər"}</button>}</div>}{pending && <div className="mt-3 rounded-md bg-[#D7B45A]/10 border border-[#D7B45A]/20 px-2.5 py-2 text-[10px] text-[#D7B45A]">Təsdiq gözləyir · {pending.label}</div>}{order.rejectedReason && <div className="mt-3 rounded-md bg-[#D77464]/10 border border-[#D77464]/20 px-2.5 py-2 text-[10px] text-[#E58A7C]">İmtina səbəbi: {order.rejectedReason}</div>}<div className="mt-4 pt-3 border-t border-white/[.07] flex items-center justify-between text-[10px] text-[#687A86]"><span className="flex items-center gap-1.5"><Clock3 size={12} />{formatDate(order.createdAt)}</span><span className="flex items-center gap-1.5 text-[#A3B1BA]" title="Cari mərhələdə keçən vaxt"><span className="h-1.5 w-1.5 rounded-full bg-[#D78A4A] proline-theme-button" />{durationLabel(order.stageEnteredAt, now)}</span></div></article> }
function OrderModal({ order, user, auditLogs, comments, commentBody, setCommentBody, onSubmitComment, onClose, onEdit }: {
  order: Order;
  user: User;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#02070B]/75 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="flex min-h-0 w-full max-w-xl max-h-[calc(100vh-1.5rem)] flex-col rounded-2xl border border-white/10 bg-[#0D1C29] proline-theme-surface shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="sticky bottom-0 shrink-0 px-4 sm:px-6 py-4 bg-[#0A1722] border-t border-white/[.08] flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3"><span className="text-[11px] text-[#697B87] flex items-center gap-1.5"><ShieldCheck size={13} /> Sifariş silinməzdir</span>{canEdit(user) && <button onClick={onEdit} className="inline-flex items-center justify-center gap-2 bg-[#D78A4A] proline-theme-button hover:bg-[#E49A5A] text-[#1B1713] px-3.5 py-2 rounded-lg text-xs font-bold"><Pencil size={14} /> Redaktə et</button>}</div>
      </div>
    </div>
  );
}
