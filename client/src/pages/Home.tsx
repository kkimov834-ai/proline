...
export default function Home() {
  ...
  return <div data-appearance={appearance} className={`proline-app min-h-screen bg-[#08121B] text-[#E9EEF2] ${appearance === "light" ? "proline-light" : ""}`} style={{ backgroundColor: appearance === "light" ? "#f4f4f2" : "#090909", "--proline-accent": "#f5f5f5", "--proline-accent-hover": "#d4d4d4", "--proline-glow": "rgba(255,255,255,.06)", "--proline-surface": "#090909" } as React.CSSProperties}>
    ...
    {settingsOpen && <SettingsPanel canExport={user.role === "admin"} appearance={appearance} setAppearance={setAppearance} templateId={excelTemplate} setTemplateId={setExcelTemplate} themeId={themeId} setThemeId={setThemeId} columnLayouts={columnLayouts} setColumnLayouts={setColumnLayouts} onExport={(format, fields) => { setSettingsOpen(false); void downloadExport(format, fields); }} onClose={() => setSettingsOpen(false)} />}
...
function SettingsPanel({ canExport, appearance, setAppearance, templateId, setTemplateId, themeId, setThemeId, columnLayouts, setColumnLayouts, onExport, onClose }: { canExport: boolean; appearance: Appearance; setAppearance: (value: Appearance) => void; templateId: ExcelTemplateId; setTemplateId: (id: ExcelTemplateId) => void; themeId: ThemeId; setThemeId: (id: ThemeId) => void; columnLayouts: Record<string, string[]>; setColumnLayouts: React.Dispatch<React.SetStateAction<Record<string, string[]>>>; onExport: (format: "csv" | "xls", fields?: string[]) => void; onClose: () => void }) {
  const activeTheme = themePresets.find((theme) => theme.id === themeId) || themePresets[0];
