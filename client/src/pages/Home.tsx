...
import { canRespondToPendingNotice, isPendingNoticeForUser } from "@shared/prolineNotifications";
...
type Notice = { id: string; orderId: string; from: ColumnId; to: ColumnId; requester: string; requesterName?: string; requesterRole?: string; createdAt: string; status: "pending" | "accepted" | "rejected"; reason?: string; };
...
type StudioColumn = { id: string; sourceId?: ColumnId; label: string; detail: string; color: string; visible: boolean };
type StudioMetric = { id: string; label: string; metric: "active" | "completed" | "urgent" | "today"; visible: boolean };
type WorkspaceConfig = { columns: StudioColumn[]; metrics: StudioMetric[]; auditPosition: "top" | "bottom" | "hidden" };
...
const defaultWorkspaceConfig = (): WorkspaceConfig => ({
  columns: columns.map((column) => ({ id: column.id, sourceId: column.id, label: column.label, detail: column.detail, color: column.color, visible: true })),
  metrics: [
    { id: "active", label: "Aktiv sifarişlər", metric: "active", visible: true },
    { id: "completed", label: "Tamamlanan", metric: "completed", visible: true },
    { id: "urgent", label: "Təcili işlər", metric: "urgent", visible: true },
    { id: "today", label: "Bu gün", metric: "today", visible: true },
  ],
  auditPosition: "top",
});
function parseWorkspaceConfig(value: string | null | undefined): WorkspaceConfig {
  try {
    const parsed = JSON.parse(value || "") as Partial<WorkspaceConfig>;
    if (!Array.isArray(parsed.columns) || !Array.isArray(parsed.metrics)) return defaultWorkspaceConfig();
    return { columns: parsed.columns, metrics: parsed.metrics, auditPosition: parsed.auditPosition === "bottom" || parsed.auditPosition === "hidden" ? parsed.auditPosition : "top" };
  } catch { return defaultWorkspaceConfig(); }
}

/* Mock data intentionally disabled: PROLINE starts with a clean board. */


function formatDate(value: string | Date) { return new Date(value).toLocaleString("az-AZ"); }
function todayLabel() { return new Date().toLocaleString("az-AZ"); }
...
export default function Home() {
  ...
  const [workspace, setWorkspace] = useState<WorkspaceConfig>(defaultWorkspaceConfig);
  ...
  const workspaceQuery = trpc.board.workspace.useQuery(undefined, { enabled: sessionReady });
  ...
  useEffect(() => { window.localStorage.setItem("proline-appearance", appearance); }, [appearance]);
  useEffect(() => { window.localStorage.setItem("proline-excel-columns", JSON.stringify(columnLayouts)); }, [columnLayouts]);
  useEffect(() => { if (workspaceQuery.data !== undefined) setWorkspace(parseWorkspaceConfig(workspaceQuery.data)); }, [workspaceQuery.data]);
  ...
  useEffect(() => { if (!boardQuery.data) return; const mappedOrders: Order[] = boardQuery.data.orders.map((item) => ({ id: String(item.id), title: item.title, description: item.description || "", image: item.imageUrl || undefined, priority: item.priority, createdAt: item.createdAt.toISOString(), column: item.columnId, stageEnteredAt: item.stageEnteredAt.toISOString(), pendingTo: item.pendingTo || undefined, rejectedReason: item.rejectedReason || undefined })); setOrders(mappedOrders); const mappedNotices: Notice[] = boardQuery.data.notifications.map((item) => ({ id: String(item.id), orderId: String(item.orderId), from: item.fromColumn, to: item.toColumn, requester: String(item.requesterUserId), requesterName: item.requesterName, requesterRole: item.requesterRole, createdAt: item.createdAt.toISOString(), status: item.status, reason: item.reason || undefined })); setNotices(mappedNotices); setStaff(boardQuery.data.staff || []); }, [boardQuery.data]);
  const allowedColumns = useMemo(() => visibleProlineColumns(user?.role || "admin") as ColumnId[], [user?.role]);
  const visibleWorkspaceColumns = useMemo(() => workspace.columns.filter((column) => column.visible && (!column.sourceId || allowedColumns.includes(column.sourceId))), [workspace.columns, allowedColumns]);
  ...
  function startPointerDrag(order: Order, event: React.PointerEvent<HTMLElement>) {
    ...
    if (!window.matchMedia("(pointer: coarse)").matches || event.button !== 0) return;
    event.stopPropagation();
    suppressNextCardClickRef.current = false;
  ...
  async function login(e: React.FormEvent) { e.preventDefault(); const normalizedEmail = email.trim().toLowerCase(); const normalizedCode = code.replace(/\D/g, "").slice(0, 6); try { const result = await loginMutation.mutateAsync({ name: name.trim(), email: normalizedEmail, code: normalizedCode }); setSessionProbeEnabled(true); setLoggingOut(false); setUser({ email: result.email, role: result.role as Role, label: result.label, column: result.column as ColumnId | undefined, userId: result.userId }); try { if (result.token) sessionStorage.setItem("proline_session_token", result.token); } catch { /* storage unavailable */ } try { const AudioContextClass = window.AudioContext || (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext; if (AudioContextClass) { audioRef.current = audioRef.current || new AudioContextClass(); await audioRef.current.resume(); audioUnlockedRef.current = true; } } catch { /* audio permission unavailable */ } setLoginError(""); toast.success(`${result.label} kimi daxil oldunuz`); } catch { setLoginError("Email və ya giriş kodu yanlışdır."); } }
  ...
  async function saveOrder(e: React.FormEvent) { e.preventDefault(); if (!form.title.trim()) return toast.error("Sifariş adı daxil edin."); try { if (selected) { await updateMutation.mutateAsync({ orderId: Number(selected.id), title: form.title.trim(), description: form.description.trim(), imageUrl: form.image, priority: form.priority }); await boardQuery.refetch(); toast.success("Sifariş yeniləndi"); } else { await createMutation.mutateAsync({ title: form.title.trim(), description: form.description.trim(), imageUrl: form.image, priority: form.priority }); await boardQuery.refetch(); } setCreating(false); } catch (error) { showOperationError(error, "Sifariş yadda saxlanmadı. Server bağlantısını yoxlayın."); } }
  async function moveOrder(order: Order, to: ColumnId) { if (to === order.column) return false; if (!canMove(user, order.column, to)) { toast.error("Bu mərhələyə keçid üçün icazəniz yoxdur."); return false; } if (moveInFlightRef.current.has(order.id)) return false; moveInFlightRef.current.add(order.id); try { await requestMoveMutation.mutateAsync({ orderId: Number(order.id), toColumn: to }); setOrders((current) => current.map((item) => item.id === order.id ? { ...item, pendingTo: to, rejectedReason: undefined } : item)); toast.success(`Təsdiq sorğusu ${columns.find((column) => column.id === to)?.label || "növbəti mərhələ"} üçün göndərildi`); void boardQuery.refetch().catch((error) => showOperationError(error, "Sifariş siyahısı yenilənmədi.")); return true; } catch (error) { showOperationError(error, "Sifariş keçidi serverdə yadda saxlanmadı."); return false; } finally { moveInFlightRef.current.delete(order.id); } }
  ...
  async function submitComment(e: React.FormEvent) { e.preventDefault(); if (!selected || !commentBody.trim()) return; try { await addCommentMutation.mutateAsync({ orderId: Number(selected.id), body: commentBody.trim() }); setCommentBody(""); await trpcUtils.board.comments.invalidate({ orderId: Number(selected.id) }); await auditFeedQuery.refetch(); toast.success("Şərh əlavə edildi"); } catch (error) { showOperationError(error, "Şərh əlavə olunmadı. Yenidən cəhd edin."); } }
  ...
  return <div data-appearance={appearance} className={`proline-app min-h-screen bg-[#08121B] text-[#E9EEF2] ${appearance === "light" ? "proline-light" : ""}`} style={{ backgroundColor: appearance === "light" ? "#f4f4f2" : "#090909", "--proline-accent": "#f5f5f5", "--proline-accent-hover": "#d4d4d4", "--proline-glow": "rgba(255,255,255,.06)", "--proline-surface": "#090909" } as React.CSSProperties}>
    <header className="min-h-[76px] border-b border-white/[.08] bg-[#0B1823]/90 backdrop-blur-xl flex items-center justify-between gap-3 px-3 sm:px-5 lg:px-9 py-3 sticky top-0 z-30">
      <div className="flex items-center gap-3"><img src="/assets/proline-logo.png" className="h-10 w-10 rounded-md object-contain" alt="PROLINE" /><div><div className="font-display text-[17px] font-bold tracking-[.16em] leading-none">PROLINE</div><div className="text-[10px] uppercase tracking-[.2em] text-[#8292A0] mt-1">Production control</div></div></div>
    ...
    <main className="proline-main px-3 sm:px-5 lg:px-9 py-5 sm:py-8 max-w-[1600px] mx-auto"><div className="mb-4 flex justify-end"><div className="w-full sm:w-auto"><button type="button" aria-expanded={onlineOpen} onClick={() => setOnlineOpen((open) => !open)} className="flex w-full items-center justify-between gap-3 rounded-lg border border-white/[.1] bg-[#0C1925]/90 px-3 py-2.5 text-left text-xs text-[#DDE5EA] sm:hidden"><span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#7FA98B] shadow-[0_0_8px_#7FA98B]" />Online işçilər <span className="text-[#AAB9C4]">({staff.filter((worker) => worker.online || worker.email === user.email).length})</span></span><ChevronDown size={15} className={`transition-transform ${onlineOpen ? "rotate-180" : ""}`} /></button><div className={`${onlineOpen ? "block" : "hidden"} mt-2 sm:hidden`}><StaffStatus current={user} staff={staff} /></div><div className="hidden sm:block"><StaffStatus current={user} staff={staff} /></div></div></div><NoticeTray notices={pendingForUser} orders={orders} user={user} onAccept={(n) => respondNotice(n, true)} onReject={(n) => { setRejecting(n); setRejectReason(""); }} />
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">{workspace.metrics.filter((metric) => metric.visible).map((metric) => {
        const values = {
          active: { value: total, detail: "bütün xətt üzrə", icon: ClipboardList, accent: "#D78A4A" },
          completed: { value: completed, detail: `${total ? Math.round((completed / total) * 100) : 0}% anbar mərhələsi`, icon: PackageCheck, accent: "#7FA98B" },
          urgent: { value: orders.filter((o) => o.priority === "urgent").length, detail: "prioritet nəzarəti", icon: Zap, accent: "#D77464" },
          today: { value: orders.filter((o) => new Date(o.createdAt).toDateString() === new Date().toDateString()).length, detail: "yeni daxil olanlar", icon: CalendarDays, accent: "#91A5B7" },
        }[metric.metric];
        return <Metric key={metric.id} label={metric.label} {...values} />;
      })}</section>
      {workspace.auditPosition === "top" && <AuditPanel logs={auditFeedQuery.data || []} />}
      ...
      <section ref={boardRef} onDragOver={handleDesktopDragOver} onPointerMove={handlePointerDragMove} onPointerUp={finishPointerDrag} onPointerCancel={cancelPointerDrag} onTouchMove={handleMobileDragMove} onTouchEnd={finishMobileDrag} onTouchCancel={cancelMobileDrag} style={{ touchAction: "pan-y", WebkitOverflowScrolling: "touch", scrollSnapType: "x proximity" }} className="proline-board flex gap-4 pb-5 overflow-x-auto overscroll-x-contain scroll-smooth snap-x">{visibleWorkspaceColumns.map((column) => { const source = column.sourceId; const list = source ? visibleOrders.filter((o) => o.column === source) : []; const Icon = columns.find((item) => item.id === source)?.icon || ClipboardList; return <div key={column.id} data-proline-column={source} onDragOver={(e) => { if (!source) return; e.preventDefault(); e.dataTransfer.dropEffect = "move"; setDropTarget(source); }} onDragLeave={(e) => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setDropTarget(null); }} onDrop={(e) => { if (!source) return; e.preventDefault(); e.stopPropagation(); const draggedId = nativeDraggedRef.current?.id || e.dataTransfer.getData("text/plain"); const order = nativeDraggedRef.current || orders.find((item) => item.id === draggedId); if (order) void moveOrder(order, source); handleNativeDragEnd(); }} className={`min-w-[280px] w-[min(100%,360px)] flex-none snap-start rounded-xl border xl:min-w-[278px] xl:w-auto xl:flex-1 bg-[#0C1925]/90 overflow-hidden transition-all duration-200 ${source && dropTarget === source ? "border-[#D78A4A] proline-theme-border shadow-[0_0_0_3px_rgba(215,138,74,.13)] -translate-y-1" : "border-white/[.09]"}`}><div className="px-4 py-4 border-b border-white/[.08]" style={{ borderTop: `2px solid ${column.color}` }}><div className="flex items-center justify-between"><div className="flex items-center gap-2.5"><span className="h-8 w-8 rounded-lg bg-white/[.055] flex items-center justify-center" style={{ color: column.color }}><Icon size={16} /></span><div><div className="font-display font-bold text-sm">{column.label}</div><div className="text-[10px] uppercase tracking-[.12em] text-[#70818E] mt-0.5">{column.detail}</div></div></div><span className="font-display text-xs font-bold text-[#A8B6C0] bg-white/[.06] rounded-md px-2 py-1">{list.length}</span></div></div><div className="p-3 min-h-[260px] space-y-3">{list.map((order) => <OrderCard key={order.id} order={order} canDrag={columns.some((target) => canMove(user, order.column, target.id))} onClick={() => { if (suppressNextCardClickRef.current) { suppressNextCardClickRef.current = false; return; } setSelected(order); }} onDragStart={(event) => handleNativeDragStart(order, event)} onDragEnd={handleNativeDragEnd} onPointerDown={(event) => startPointerDrag(order, event)} onPointerMove={handlePointerDragMove} onPointerUp={finishPointerDrag} onPointerCancel={() => {}} onTouchStart={(event) => startMobileDrag(order, event)} onTouchMove={handleMobileDragMove} onTouchEnd={finishMobileDrag} onTouchCancel={cancelMobileDrag} />)}{list.length === 0 && <div className="h-[220px] rounded-lg border border-dashed border-white/10 flex flex-col items-center justify-center text-center px-4"><div className="h-10 w-10 rounded-full bg-white/[.035] flex items-center justify-center mb-3"><ArrowRight size={16} className="text-[#536571]" /></div><div className="text-xs text-[#647582]">{source ? "Sifarişi bura\nsürüklə" : "Studyo sütunu\nKart gözləyir"}</div></div>}</div></div> })}{user.role === "admin" && <button type="button" onClick={() => setStudioOpen(true)} className="min-w-[280px] min-h-[260px] flex-none snap-start rounded-xl border border-dashed border-[#D78A4A]/45 bg-[#D78A4A]/[.04] p-6 text-center text-[#DDB083] transition-all duration-200 hover:-translate-y-1 hover:border-[#D78A4A] hover:bg-[#D78A4A]/[.09] active:scale-[.98]"><Plus size={22} className="mx-auto mb-3" /><span className="block text-sm font-bold">Yeni Sütun Əlavə Edin</span><span className="mt-2 block text-[11px] text-[#9A897A]">Admin üçün sütun və rol ayarları</span></button>}</section>
      {workspace.auditPosition === "bottom" && <AuditPanel logs={auditFeedQuery.data || []} />}
    ...
    {settingsOpen && <SettingsPanel canExport={user.role === "admin"} onStudio={user.role === "admin" ? () => { setSettingsOpen(false); setStudioOpen(true); } : undefined} appearance={appearance} setAppearance={setAppearance} templateId={excelTemplate} setTemplateId={setExcelTemplate} themeId={themeId} setThemeId={setThemeId} columnLayouts={columnLayouts} setColumnLayouts={setColumnLayouts} onExport={(format, fields) => { setSettingsOpen(false); void downloadExport(format, fields); }} onClose={() => setSettingsOpen(false)} />}
    {studioOpen && <StudioEditor initial={workspace} saving={saveWorkspaceMutation.isPending} onClose={() => setStudioOpen(false)} onSave={async (next) => { try { await saveWorkspaceMutation.mutateAsync({ config: JSON.stringify(next) }); setWorkspace(next); setStudioOpen(false); toast.success("Studyo dəyişiklikləri workspace-ə tətbiq edildi."); } catch (error) { showOperationError(error, "Studyo dəyişiklikləri yadda saxlanmadı."); } }} />}
...
function LoginScreen({ name, email, code, setName, setEmail, setCode, error, onSubmit }: { name: string; email: string; code: string; setName: (v: string) => void; setEmail: (v: string) => void; setCode: (v: string) => void; error: string; onSubmit: (e: React.FormEvent) => void }) { return <div className="min-h-screen bg-[#08121B] text-[#E9EEF2] flex items-center justify-center px-5 relative overflow-hidden" style={{ backgroundImage: "linear-gradient(rgba(8,18,27,.86), rgba(8,18,27,.96)), url('/manus-storage/proline-grid_53b97e55.png')", backgroundSize: "cover" }}><div className="absolute -top-32 -right-24 h-96 w-96 rounded-full bg-[#D78A4A] bg-[#D78A4A]/10 blur-3xl" /><div className="w-full max-w-[980px] grid lg:grid-cols-[1.1fr_.9fr] border border-white/[.14] rounded-2xl overflow-hidden bg-[#0B1823]/90 backdrop-blur-xl shadow-[0_28px_90px_rgba(0,0,0,.42)] relative ring-1 ring-[#D78A4A]/10"><div className="hidden lg:flex flex-col justify-between p-12 border-r border-white/[.08] min-h-[570px]" style={{ backgroundImage: "linear-gradient(130deg, rgba(10,25,36,.45), rgba(10,25,36,.92)), url('/manus-storage/proline-empty-state_07d8bf97.png')", backgroundSize: "cover", backgroundPosition: "center" }}><div><div className="flex items-center gap-3"><img src="/assets/proline-logo.png" className="h-11 w-11 rounded-md object-contain" alt="PROLINE" /><span className="font-display text-lg font-bold tracking-[.16em]">PROLINE</span></div><div className="mt-24 max-w-sm"><div className="text-[#D78A4A] proline-theme-accent text-[11px] font-bold uppercase tracking-[.2em] mb-4">Production control</div><h1 className="font-display text-5xl font-bold leading-[1.02] tracking-[-.05em]">İstehsalatın<br /><span className="text-[#D78A4A] proline-theme-accent">növbəti xətti.</span></h1><p className="mt-5 text-[#A6B3BC] text-sm leading-6">Sifarişdən anbara qədər hər addımı bir səthdə görünən, ciddi və çevik idarəetmə mühiti.</p></div></div><div className="flex items-center gap-2 text-xs text-[#8292A0]"><span className="h-2 w-2 rounded-full bg-[#7FA98B]" /> Təhlükəsiz iş sahəsi · 5 mərhələ</div></div><div className="p-8 sm:p-12 bg-[linear-gradient(140deg,rgba(18,34,48,.28),transparent_42%)] relative"><div className="absolute top-0 right-0 h-px w-32 bg-gradient-to-l from-[#D78A4A]/60 to-transparent" /><div className="lg:hidden flex items-center gap-3 mb-14"><img src="/assets/proline-logo.png" className="h-10 w-10 rounded-md object-contain" alt="PROLINE" /><span className="font-display font-bold tracking-[.16em]">PROLINE</span></div><div className="flex items-center justify-between border-b border-white/[.08] pb-4 mb-9"><div className="text-[10px] uppercase tracking-[.18em] text-[#718391] flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#7FA98B]" /> Access bay / 01</div><div className="font-display text-[10px] tracking-[.15em] text-[#D78A4A] proline-theme-accent">PL-LOGIN</div></div><div className="mb-9"><div className="text-[#D78A4A] proline-theme-accent text-[10px] font-bold uppercase tracking-[.22em] mb-3">Operator identifikasiyası</div><h2 className="font-display text-3xl font-bold tracking-[-.045em]">İş sahəsinə daxil ol</h2><p className="text-[#81919E] text-sm mt-3 leading-5">Roluna uyğun əməliyyat panelini aç.</p></div><form onSubmit={onSubmit} className="space-y-5"><label className="block"><span className="text-[10px] uppercase tracking-[.14em] font-bold text-[#B6C1C9]">Adınız</span><input value={name} onChange={(e) => setName(e.target.value)} autoComplete="name" aria-label="İstifadəçi adı" required className="mt-2 w-full rounded-xl border border-white/[.16] bg-[#061018] px-3.5 py-3 text-sm text-[#E8EEF2] outline-none focus:border-[#D78A4A] proline-theme-border focus:ring-2 focus:ring-[#D78A4A]/20 transition" /></label><label className="block"><span className="text-[10px] uppercase tracking-[.14em] font-bold text-[#B6C1C9]">Email identifikatoru</span><input value={email} onChange={(e) => setEmail(e.target.value.toLowerCase())} autoComplete="username" aria-label="Sifariş hesab identifikatoru" className="mt-2 w-full rounded-xl border border-white/[.16] bg-[#061018] px-3.5 py-3 text-sm text-[#E8EEF2] outline-none focus:border-[#D78A4A] proline-theme-border focus:ring-2 focus:ring-[#D78A4A]/20 transition" /></label><label className="block"><span className="text-[10px] uppercase tracking-[.14em] font-bold text-[#B6C1C9]">Giriş kodu / access key</span><input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))} autoComplete="current-password" type="password" inputMode="numeric" maxLength={6} aria-label="Giriş kodu" className="mt-2 w-full rounded-xl border border-white/[.16] bg-[#061018] px-3.5 py-3 text-sm tracking-[.35em] text-[#E8EEF2] outline-none focus:border-[#D78A4A] proline-theme-border focus:ring-2 focus:ring-[#D78A4A]/20 transition" /></label>{error && <div className="text-xs text-[#E58A7C] bg-[#D77464]/10 border border-[#D77464]/20 px-3 py-2 rounded-lg">{error}</div>}<button type="submit" className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#D78A4A] proline-theme-button hover:bg-[#E49A5A] text-[#1B1713] py-3 text-sm font-bold transition active:scale-[.98]">Panelə daxil ol <LogIn size={16} /></button></form><div className="mt-8 pt-5 border-t border-white/[.08] flex gap-2 text-[11px] text-[#687984]"><CircleHelp size={14} className="shrink-0 text-[#D78A4A] proline-theme-accent" /><span>Giriş yalnız əvvəlcədən təyin edilmiş PROLINE rolları üçün açıqdır.</span></div><div className="mt-7 flex items-center gap-1.5"><span className="text-[9px] uppercase tracking-[.16em] text-[#536571] mr-1">Workflow</span>{[0,1,2,3,4].map((item) => <span key={item} className={`h-1 flex-1 rounded-full ${item === 0 ? "bg-[#D78A4A] proline-theme-button" : "bg-[#243743]"}`} />)}</div></div></div></div> }
...
function AuditPanel({ logs }: { logs: { id: number; action: string; details: string | null; orderId: number | null; createdAt: Date }[] }) { if (!logs.length) return null; return <section className="proline-audit mb-6 rounded-xl border border-white/[.09] bg-[#0C1925]/90 p-4"><div className="flex items-center gap-2 mb-3"><History size={15} className="text-[#D78A4A] proline-theme-accent" /><div className="font-display font-bold text-sm">Son fəaliyyətlər</div><span className="text-[10px] uppercase tracking-[.14em] text-[#6F818E]">audit log</span></div><div className="grid md:grid-cols-2 xl:grid-cols-4 gap-2">{logs.slice(0, 8).map((log) => <div key={`${log.id}-${log.createdAt.toISOString()}`} className="rounded-lg bg-white/[.035] px-3 py-2"><div className="text-[10px] font-semibold text-[#C0CBD2]">{log.details || log.action}</div><div className="text-[10px] text-[#718391] mt-1">{log.orderId ? `Sifariş #${log.orderId}` : "Sistem"} · {formatDate(log.createdAt.toISOString())}</div></div>)}</div></section>; }
...
function VisualStudioMode({ initial, saving, onSave, onClose }: { initial: WorkspaceConfig; saving: boolean; onSave: (config: WorkspaceConfig) => void; onClose: () => void }) {
  ...
  const receive = (event: React.DragEvent, target?: Selected) => {
    ...
    if (palette) { add(palette); return; }
    if (!target || !item) return;
    const [kind, id] = item.split(":");
    if (kind === "column" && target.kind === "column") setDraft((value) => ({ ...value, columns: reorder(value.columns, id, target.id) }));
    if (kind === "metric" && target.kind === "metric") setDraft((value) => ({ ...value, metrics: reorder(value.metrics, id, target.id) }));
  };
  const updateSelected = (patch: Record<string, unknown>) => {
    if (!selected || selected.kind === "audit") return;
    setDraft((value) => selected.kind === "column" ? { ...value, columns: value.columns.map((item) => item.id === selected.id ? { ...item, ...patch } : item) } : { ...value, metrics: value.metrics.map((item) => item.id === selected.id ? { ...item, ...patch } : item) });
  };
  const removeSelected = () => {
    if (!selected) return;
    if (selected.kind === "audit") setDraft((value) => ({ ...value, auditPosition: "hidden" }));
    else setDraft((value) => selected.kind === "column" ? { ...value, columns: value.columns.filter((item) => item.id !== selected.id) } : { ...value, metrics: value.metrics.filter((item) => item.id !== selected.id) });
    setSelected(null);
  };
  const selectedItem = selected?.kind === "column" ? draft.columns.find((item) => item.id === selected.id) : selected?.kind === "metric" ? draft.metrics.find((item) => item.id === selected.id) : undefined;
  const palette = [{ kind: "column", title: "Sütun", text: "İş sahəsinə sütun əlavə et", icon: SlidersHorizontal }, { kind: "metric", title: "Məlumat kartı", text: "Canlı göstərici kartı", icon: ClipboardList }, { kind: "audit", title: "Audit log", text: "Fəaliyyət paneli", icon: History }];
  return <div className="studio-shell fixed inset-0 z-[80] flex flex-col bg-[#111318] text-[#E8EEF2]">
    <header className="flex min-h-14 shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-[#191B20] px-3 sm:px-5"><div className="flex min-w-0 items-center gap-3"><span className="font-display text-sm font-bold tracking-wide">STUDYO MOD</span><span className="hidden text-xs text-[#8E99A8] sm:inline">Workspace visual qurucusu</span><span className="hidden rounded bg-[#1E6EBE]/20 px-2 py-1 text-[10px] text-[#76C5FF] md:inline">Qaralama</span></div><div className="flex items-center gap-2"><button onClick={onClose} className="rounded border border-white/15 px-3 py-2 text-xs font-bold hover:bg-white/5">Bağla</button><button disabled={saving} onClick={() => onSave(draft)} className="inline-flex items-center gap-2 rounded bg-[#1677D2] px-3 py-2 text-xs font-bold text-white disabled:opacity-60"><Check size={14} /> {saving ? "Saxlanır..." : "Save Global (Studio)"}</button></div></header>
    <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(220px,280px)_minmax(0,1fr)_minmax(250px,310px)]">
      <aside className="order-3 overflow-y-auto border-t border-white/10 bg-[#181A1F] p-4 lg:order-1 lg:border-r lg:border-t-0"><div className="text-[10px] font-bold uppercase tracking-[.14em] text-[#7E8A99]">Seçilən element</div>{!selected && <div className="mt-5 rounded-lg border border-dashed border-white/15 p-4 text-xs leading-5 text-[#8E99A8]">Ortadakı önizləmədə sütun, kart və ya audit blokuna klik edin. Ad, görünüş və silmə ayarları burada açılacaq.</div>}{selected?.kind === "audit" && <div className="mt-4 space-y-3"><div className="text-sm font-bold">Audit log</div><label className="block text-xs text-[#AAB5C2]">Yer<select value={draft.auditPosition} onChange={(event) => setDraft((value) => ({ ...value, auditPosition: event.target.value as WorkspaceConfig["auditPosition"] }))} className="mt-1.5 w-full rounded border border-white/15 bg-[#101217] px-2 py-2 text-xs"><option value="top">Yuxarı</option><option value="bottom">Aşağı</option><option value="hidden">Gizli</option></select></label><button onClick={removeSelected} className="inline-flex items-center gap-2 rounded border border-red-400/35 px-3 py-2 text-xs font-bold text-red-300 hover:bg-red-400/10"><Trash2 size={14} /> Audit log-u sil</button></div>}{selectedItem && <div className="mt-4 space-y-3"><div className="flex items-center justify-between"><div className="text-sm font-bold">{selected?.kind === "column" ? "Sütun ayarları" : "Kart ayarları"}</div><button onClick={removeSelected} className="rounded p-1.5 text-red-300 hover:bg-red-400/10" title="Sil"><Trash2 size={15} /></button></div><label className="block text-xs text-[#AAB5C2]">Ad<input autoFocus value={selectedItem.label} onChange={(event) => updateSelected({ label: event.target.value })} className="mt-1.5 w-full rounded border border-white/15 bg-[#101217] px-2 py-2 text-xs text-white" /></label>{selected?.kind === "column" ? <><label className="block text-xs text-[#AAB5C2]">Açıqlama<input value={(selectedItem as StudioColumn).detail} onChange={(event) => updateSelected({ detail: event.target.value })} className="mt-1.5 w-full rounded border border-white/15 bg-[#101217] px-2 py-2 text-xs text-white" /></label><label className="flex items-center justify-between text-xs text-[#AAB5C2]">Rəng<input type="color" value={(selectedItem as StudioColumn).color} onChange={(event) => updateSelected({ color: event.target.value })} className="h-8 w-12 rounded border border-white/15 bg-transparent" /></label></> : <label className="block text-xs text-[#AAB5C2]">Məlumat<select value={(selectedItem as StudioMetric).metric} onChange={(event) => updateSelected({ metric: event.target.value })} className="mt-1.5 w-full rounded border border-white/15 bg-[#101217] px-2 py-2 text-xs"><option value="active">Aktiv sifarişlər</option><option value="completed">Tamamlanan</option><option value="urgent">Təcili işlər</option><option value="today">Bu gün</option></select></label>}<label className="flex items-center gap-2 text-xs text-[#AAB5C2]"><input type="checkbox" checked={selectedItem.visible} onChange={(event) => updateSelected({ visible: event.target.checked })} /> Workspace-də görünür</label></div>}</aside>
      <main onDragOver={(event) => event.preventDefault()} onDrop={(event) => receive(event)} className="order-1 min-w-0 overflow-auto bg-[#101217] p-4 sm:p-7 lg:order-2"><div className="mx-auto max-w-5xl"><div className="mb-4 flex items-center justify-between"><div><div className="text-[10px] font-bold uppercase tracking-[.15em] text-[#8593A3]">Canlı önizləmə</div><h2 className="mt-1 font-display text-xl font-bold">İstehsalat workspace</h2></div><span className="text-xs text-[#8190A0]">Elementi seçin · sürükləyərək yerini dəyişin</span></div><section className="grid grid-cols-2 gap-3 xl:grid-cols-4">{draft.metrics.filter((item) => item.visible).map((item) => <button key={item.id} draggable onDragStart={(event) => event.dataTransfer.setData("studio-item", `metric:${item.id}`)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => receive(event, { kind: "metric", id: item.id })} onClick={() => setSelected({ kind: "metric", id: item.id })} className={`rounded-lg border p-3 text-left transition ${selected?.kind === "metric" && selected.id === item.id ? "border-[#38BDF8] ring-2 ring-[#38BDF8]/25" : "border-white/10 bg-[#1B1E25] hover:border-white/30"}`}><div className="flex items-center gap-2 text-[#6EC7FF]"><ClipboardList size={15} /><span className="text-[10px] font-bold uppercase tracking-wide">Kart</span></div><div className="mt-3 text-sm font-bold">{item.label}</div><div className="mt-1 text-xl font-display font-bold">123</div></button>)}</section><section className="mt-5"><div className="mb-2 text-[10px] font-bold uppercase tracking-[.15em] text-[#8593A3]">Sütunların önizləməsi</div><div className="studio-canvas-drop flex min-h-80 gap-3 overflow-x-auto rounded-xl border-2 border-dashed border-[#2A78B9] bg-[#151820] p-3">{draft.columns.filter((item) => item.visible).map((item) => <button key={item.id} draggable onDragStart={(event) => event.dataTransfer.setData("studio-item", `column:${item.id}`)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => receive(event, { kind: "column", id: item.id })} onClick={() => setSelected({ kind: "column", id: item.id })} className={`min-w-56 flex-1 rounded-lg border bg-[#1B1E25] text-left transition ${selected?.kind === "column" && selected.id === item.id ? "border-[#38BDF8] ring-2 ring-[#38BDF8]/25" : "border-white/10 hover:border-white/30"}`}><div className="border-t-2 p-3" style={{ borderTopColor: item.color }}><div className="flex items-center gap-2"><GripVertical size={15} className="text-[#748092]" /><span className="font-bold">{item.label}</span></div><div className="mt-1 text-[10px] uppercase tracking-wide text-[#7C8998]">{item.detail}</div></div><div className="m-3 rounded border border-dashed border-white/10 p-4 text-center text-[11px] text-[#7E8B9A]">Kartlar üçün sahə</div></button>)}{draft.columns.filter((item) => item.visible).length === 0 && <div className="flex w-full items-center justify-center text-sm text-[#8C99A8]">Sağdakı “Sütun” elementini buraya sürükləyin.</div>}</div></section>{draft.auditPosition !== "hidden" && <button draggable onDragStart={(event) => event.dataTransfer.setData("studio-item", "audit:audit")} onClick={() => setSelected({ kind: "audit" })} className={`mt-5 w-full rounded-lg border p-4 text-left ${selected?.kind === "audit" ? "border-[#38BDF8] ring-2 ring-[#38BDF8]/25" : "border-white/10 bg-[#1B1E25]"}`}><div className="flex items-center gap-2"><History size={16} className="text-[#6EC7FF]" /><span className="font-bold">Audit log</span><span className="ml-auto text-[10px] text-[#8D99A7]">{draft.auditPosition === "top" ? "Yuxarı" : "Aşağı"}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><span className="rounded bg-white/[.04] px-2 py-2 text-xs text-[#9BA7B5]">Sifariş yaradıldı</span><span className="rounded bg-white/[.04] px-2 py-2 text-xs text-[#9BA7B5]">Mərhələ dəyişdi</span><span className="rounded bg-white/[.04] px-2 py-2 text-xs text-[#9BA7B5]">Şərh əlavə edildi</span></div></button>}</div></main>
      <aside className="order-2 overflow-y-auto border-y border-white/10 bg-[#181A1F] p-4 lg:order-3 lg:border-x-0 lg:border-y-0 lg:border-l"><div className="text-[10px] font-bold uppercase tracking-[.14em] text-[#7E8A99]">Əlavə et</div><p className="mt-2 text-xs leading-5 text-[#98A4B2]">Elementi tutub ortadakı sahəyə sürükləyin.</p><div className="mt-4 space-y-2">{palette.map(({ kind, title, text, icon: Icon }) => <button key={kind} draggable onDragStart={(event) => event.dataTransfer.setData("studio-palette", kind)} onClick={() => add(kind)} className="flex w-full items-center gap-3 rounded-lg border border-white/10 bg-[#20232B] p-3 text-left hover:border-[#38BDF8]/70 hover:bg-[#252A33]"><span className="flex h-8 w-8 items-center justify-center rounded bg-[#1677D2]/15 text-[#54B6F7]"><Icon size={16} /></span><span><span className="block text-xs font-bold">{title}</span><span className="mt-0.5 block text-[10px] text-[#8D99A8]">{text}</span></span></button>)}</div><div className="mt-6 rounded-lg border border-white/10 bg-[#14161B] p-3 text-[11px] leading-5 text-[#94A0AE]"><strong className="text-[#D7E2EC]">İpucu:</strong> Önizləmədə elementləri bir-birinin üzərinə ataraq sıralayın. Kliklə sol paneldə adını, rəngini və görünməsini dəyişin.</div></aside>
    </div>
  </div>;
}

function StudioMode({ initial, saving, onSave, onClose }: { initial: WorkspaceConfig; saving: boolean; onSave: (config: WorkspaceConfig) => void; onClose: () => void }) {
  const [draft, setDraft] = useState<WorkspaceConfig>(() => structuredClone(initial));
  const updateColumn = (id: string, patch: Partial<StudioColumn>) => setDraft((value) => ({ ...value, columns: value.columns.map((column) => column.id === id ? { ...column, ...patch } : column) }));
  const updateMetric = (id: string, patch: Partial<StudioMetric>) => setDraft((value) => ({ ...value, metrics: value.metrics.map((metric) => metric.id === id ? { ...metric, ...patch } : metric) }));
  const move = <T extends { id: string }>(items: T[], id: string, delta: number) => { const index = items.findIndex((item) => item.id === id); const target = index + delta; if (index < 0 || target < 0 || target >= items.length) return items; const next = [...items]; [next[index], next[target]] = [next[target], next[index]]; return next; };
  return <div className="fixed inset-0 z-[80] overflow-y-auto bg-[#02070B]/85 p-3 backdrop-blur-sm sm:p-6"><div className="mx-auto max-w-5xl rounded-2xl border border-white/10 bg-[#0D1C29] shadow-2xl"><div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-white/[.08] bg-[#0D1C29] px-4 py-4 sm:px-6"><div><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#D78A4A]"><SlidersHorizontal size={13} /> Studyo mod · qaralama</div><h2 className="mt-2 font-display text-xl font-bold sm:text-2xl">Workspace nüsxəsini redaktə edin</h2><p className="mt-1 text-xs text-[#AAB9C4]">Buradakı dəyişikliklər yalnız “Yadda saxla” seçiləndə bütün istifadəçilərin workspace-inə tətbiq olunur.</p></div><button onClick={onClose} className="rounded-lg p-2 text-[#B7C3CB] hover:bg-white/10" aria-label="Studyo modunu bağla"><X size={18} /></button></div><div className="space-y-5 p-4 sm:p-6"><section className="rounded-xl border border-white/[.1] p-4"><div className="mb-3 flex items-center justify-between"><div><h3 className="font-bold">Sütunlar</h3><p className="mt-1 text-xs text-[#91A0AA]">Ad, görünüş, görünmə və sıralama. Əlavə sütunlar təqdimat üçündür; iş axını mərhələləri sabit qalır.</p></div><button type="button" onClick={() => setDraft((value) => ({ ...value, columns: [...value.columns, { id: `studio-${Date.now()}`, label: "Yeni sütun", detail: "Studyo sütunu", color: "#D78A4A", visible: true }] }))} className="inline-flex items-center gap-1 rounded-lg border border-white/15 px-3 py-2 text-xs font-bold hover:bg-white/10"><Plus size={14} /> Sütun əlavə et</button></div><div className="space-y-2">{draft.columns.map((column, index) => <div key={column.id} className="grid gap-2 rounded-lg border border-white/[.08] bg-white/[.025] p-3 md:grid-cols-[auto_1fr_1fr_100px_auto]"><input type="color" value={column.color} onChange={(event) => updateColumn(column.id, { color: event.target.value })} title="Sütun rəngi" className="h-9 w-full rounded border border-white/10 bg-transparent md:w-10" /><input value={column.label} onChange={(event) => updateColumn(column.id, { label: event.target.value })} aria-label="Sütun adı" className="rounded-md border border-white/10 bg-[#07121B] px-2 py-2 text-xs" /><input value={column.detail} onChange={(event) => updateColumn(column.id, { detail: event.target.value })} aria-label="Sütun açıqlaması" className="rounded-md border border-white/10 bg-[#07121B] px-2 py-2 text-xs" /><label className="flex items-center gap-2 text-xs text-[#C5D0D6]"><input type="checkbox" checked={column.visible} onChange={(event) => updateColumn(column.id, { visible: event.target.checked })} /> Görünür</label><div className="flex items-center justify-end gap-1"><button disabled={index === 0} onClick={() => setDraft((value) => ({ ...value, columns: move(value.columns, column.id, -1) }))} className="rounded p-1.5 hover:bg-white/10 disabled:opacity-30" aria-label="Yuxarı"><ChevronDown size={15} className="rotate-180" /></button><button disabled={index === draft.columns.length - 1} onClick={() => setDraft((value) => ({ ...value, columns: move(value.columns, column.id, 1) }))} className="rounded p-1.5 hover:bg-white/10 disabled:opacity-30" aria-label="Aşağı"><ChevronDown size={15} /></button><button onClick={() => setDraft((value) => ({ ...value, columns: value.columns.filter((item) => item.id !== column.id) }))} className="rounded p-1.5 text-[#E58A7C] hover:bg-[#D77464]/15" aria-label="Sütunu sil"><Trash2 size={15} /></button></div></div>)}</div></section><section className="rounded-xl border border-white/[.1] p-4"><div className="mb-3 flex items-center justify-between"><div><h3 className="font-bold">Məlumat kartları</h3><p className="mt-1 text-xs text-[#91A0AA]">Kartı əlavə edin, adını dəyişin, gizlədin və ya silin.</p></div><button type="button" onClick={() => setDraft((value) => ({ ...value, metrics: [...value.metrics, { id: `metric-${Date.now()}`, label: "Yeni məlumat kartı", metric: "active", visible: true }] }))} className="inline-flex items-center gap-1 rounded-lg border border-white/15 px-3 py-2 text-xs font-bold hover:bg-white/10"><Plus size={14} /> Kart əlavə et</button></div><div className="space-y-2">{draft.metrics.map((metric, index) => <div key={metric.id} className="grid gap-2 rounded-lg border border-white/[.08] bg-white/[.025] p-3 md:grid-cols-[1fr_180px_auto_auto]"><input value={metric.label} onChange={(event) => updateMetric(metric.id, { label: event.target.value })} className="rounded-md border border-white/10 bg-[#07121B] px-2 py-2 text-xs" aria-label="Kart adı" /><select value={metric.metric} onChange={(event) => updateMetric(metric.id, { metric: event.target.value as StudioMetric["metric"] })} className="rounded-md border border-white/10 bg-[#07121B] px-2 py-2 text-xs"><option value="active">Aktiv sifarişlər</option><option value="completed">Tamamlanan</option><option value="urgent">Təcili işlər</option><option value="today">Bu gün</option></select><label className="flex items-center gap-2 text-xs text-[#C5D0D6]"><input type="checkbox" checked={metric.visible} onChange={(event) => updateMetric(metric.id, { visible: event.target.checked })} /> Görünür</label><div className="flex items-center justify-end gap-1"><button disabled={index === 0} onClick={() => setDraft((value) => ({ ...value, metrics: move(value.metrics, metric.id, -1) }))} className="rounded p-1.5 hover:bg-white/10 disabled:opacity-30"><ChevronDown size={15} className="rotate-180" /></button><button disabled={index === draft.metrics.length - 1} onClick={() => setDraft((value) => ({ ...value, metrics: move(value.metrics, metric.id, 1) }))} className="rounded p-1.5 hover:bg-white/10 disabled:opacity-30"><ChevronDown size={15} /></button><button onClick={() => setDraft((value) => ({ ...value, metrics: value.metrics.filter((item) => item.id !== metric.id) }))} className="rounded p-1.5 text-[#E58A7C] hover:bg-[#D77464]/15"><Trash2 size={15} /></button></div></div>)}</div></section><section className="rounded-xl border border-white/[.1] p-4"><h3 className="font-bold">Audit log</h3><p className="mt-1 text-xs text-[#91A0AA]">Audit blokunu yuxarıda, aşağıda göstərin və ya workspace-dən gizlədin.</p><div className="mt-3 flex flex-wrap gap-2">{(["top", "bottom", "hidden"] as const).map((position) => <button key={position} onClick={() => setDraft((value) => ({ ...value, auditPosition: position }))} className={`rounded-lg border px-3 py-2 text-xs font-bold ${draft.auditPosition === position ? "border-[#D78A4A] bg-[#D78A4A]/15 text-white" : "border-white/10 text-[#AAB9C4] hover:bg-white/5"}`}>{position === "top" ? "Yuxarı" : position === "bottom" ? "Aşağı" : "Gizlət"}</button>)}</div></section></div><div className="sticky bottom-0 flex flex-col-reverse gap-2 border-t border-white/[.08] bg-[#0D1C29] px-4 py-4 sm:flex-row sm:justify-end sm:px-6"><button onClick={onClose} className="rounded-lg px-4 py-2.5 text-xs font-bold text-[#C5D0D6] hover:bg-white/5">Ləğv et</button><button disabled={saving} onClick={() => onSave(draft)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#D78A4A] px-4 py-2.5 text-xs font-bold text-[#1B1713] disabled:opacity-60"><Check size={15} /> {saving ? "Yadda saxlanır..." : "Yadda saxla və tətbiq et"}</button></div></div></div>;
}

function SettingsPanel({ canExport, onStudio, appearance, setAppearance, templateId, setTemplateId, themeId, setThemeId, columnLayouts, setColumnLayouts, onExport, onClose }: { canExport: boolean; onStudio?: () => void; appearance: Appearance; setAppearance: (value: Appearance) => void; templateId: ExcelTemplateId; setTemplateId: (id: ExcelTemplateId) => void; themeId: ThemeId; setThemeId: (id: ThemeId) => void; columnLayouts: Record<string, string[]>; setColumnLayouts: React.Dispatch<React.SetStateAction<Record<string, string[]>>>; onExport: (format: "csv" | "xls", fields?: string[]) => void; onClose: () => void }) {
  const activeTheme = themePresets.find((theme) => theme.id === themeId) || themePresets[0];
  const selectedTemplate = excelTemplates.find((template) => template.id === templateId) || excelTemplates[0];
  const [themesOpen, setThemesOpen] = useState(false);
  const [templatesOpen, setTemplatesOpen] = useState(false);
  const [settingsTab, setSettingsTab] = useState<"appearance" | "import">("appearance");
  ...
  return <div className="fixed inset-0 z-[60] bg-[#02070B]/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <div className="w-full max-w-2xl max-h-[calc(100vh-2rem)] overflow-y-auto rounded-2xl border border-white/10 bg-[#0D1C29] proline-theme-surface shadow-2xl">
      ...
      <div className="border-b border-white/[.08] px-4 sm:px-6 pt-3"><div className="grid grid-cols-2 gap-1 rounded-lg bg-black/15 p-1"><button type="button" onClick={() => setSettingsTab("appearance")} className={`rounded-md px-3 py-2.5 text-xs font-bold transition ${settingsTab === "appearance" ? "bg-white/[.1] text-white shadow-sm" : "text-[#8596A2] hover:text-white"}`}>Görünüş</button><button type="button" onClick={() => setSettingsTab("import")} className={`rounded-md px-3 py-2.5 text-xs font-bold transition ${settingsTab === "import" ? "bg-white/[.1] text-white shadow-sm" : "text-[#8596A2] hover:text-white"}`}>İmport</button></div></div>
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        {onStudio && <section className="rounded-xl border border-[#D78A4A]/30 bg-[#D78A4A]/[.06] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-[10px] uppercase tracking-[.14em] font-bold text-[#D78A4A]">Yalnız Admin</div><div className="mt-1 text-sm font-bold text-white">Studyo mod</div><p className="mt-1 text-xs leading-5 text-[#B7C3CB]">Workspace nüsxəsində sütunları, məlumat kartlarını və audit log-u redaktə edin.</p></div><button type="button" onClick={onStudio} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-[#D78A4A] px-3.5 py-2.5 text-xs font-bold text-[#1B1713]"><SlidersHorizontal size={14} /> Studyo modu aç</button></div></section>}
        ...
        <section className="proline-excel-preview rounded-xl border border-[#b7c5d1] bg-white overflow-hidden text-[#17212b]">
          <div className="flex flex-col gap-3 border-b border-white/[.08] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-[10px] uppercase tracking-[.14em] font-bold text-[#173247]">Excel önizləməsi</div><div className="text-xs text-[#52616d] mt-1">“{selectedTemplate.name}” formatında standart Excel görünüşü.</div></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-[#eaf0f4] px-2 py-1 text-[10px] font-bold text-[#52616d]">Nümunə</span><select value={downloadFormat} onChange={(event) => setDownloadFormat(event.target.value as "csv" | "xls")} aria-label="Önizləmə yükləmə formatı" className="rounded-md border border-white/15 bg-[#07121B] px-2 py-1.5 text-[10px] font-bold text-[#E8EEF2] outline-none" style={{ borderColor: `${activeTheme.accent}66` }}><option value="xls">XLS</option><option value="csv">CSV</option></select><button type="button" onClick={() => onExport(downloadFormat, previewFields)} className="inline-flex items-center justify-center gap-1.5 rounded-md bg-[#217346] px-3 py-1.5 text-[10px] font-bold text-white"><Download size={13} /> Yüklə</button></div></div>
          <div className="overflow-x-auto"><table className="min-w-full text-left text-[10px]"><thead><tr className="bg-[#217346] text-white">{previewFields.map((field) => <th key={field} className="whitespace-nowrap px-3 py-2 font-bold">{field}</th>)}</tr></thead><tbody>{previewRows.map((row, index) => <tr key={index} className={`border-b border-[#d9e2e8] last:border-0 ${index % 2 ? "bg-[#f2f7f4]" : "bg-white"}`}>{previewFields.map((field, fieldIndex) => <td key={field} className={`max-w-[220px] whitespace-nowrap px-3 py-2 ${fieldIndex === 0 ? "font-semibold text-[#173247]" : "text-[#273944]"}`}>{previewValue(row, field)}</td>)}</tr>)}</tbody></table></div><div className="border-t border-[#d9e2e8] px-4 py-2 text-[10px] text-[#52616d]">Bu yalnız önizləmədir; yüklənən faylda real sifariş məlumatları istifadə olunacaq.</div>
        </section>
        </>}
      </div>

      {settingsTab === "import" && canExport && <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-white/[.08] bg-[#0A1722] px-4 sm:px-6 py-4">
