...
import { canProlineRoleMove, nextProlineColumn, PROLINE_COLUMN_ORDER, PROLINE_ROLE_MAP, visibleProlineColumns } from "@shared/prolineAuth";
...
import { autoScrollDelta, autoScrollViewportDelta, clampScrollLeft } from "@shared/mobileBoard";
...
export default function Home() {
  ...
  const visibleOrders = useMemo(() => orders.filter((o) => { const matchesText = `${o.title} ${o.id} ${o.description}`.toLowerCase().includes(query.toLowerCase()); const matchesStatus = statusFilter === "all" || o.column === statusFilter; const created = new Date(o.createdAt); const matchesFrom = !dateFrom || created >= new Date(`${dateFrom}T00:00:00`); const matchesTo = !dateTo || created <= new Date(`${dateTo}T23:59:59`); return visibleColumns.includes(o.column) && matchesText && matchesStatus && matchesFrom && matchesTo; }), [orders, query, statusFilter, dateFrom, dateTo, visibleColumns]);
  ...
  async function logout() { if (loggingOut) return; setLoggingOut(true); setUser(null); setOrders([]); setNotices([]); setStaff([]); setSelected(null); setCreating(false); setSettingsOpen(false); setPushSettingsOpen(false); setCommentBody(""); try { await logoutMutation.mutateAsync(); await Promise.all([trpcUtils.board.session.reset(), trpcUtils.board.list.reset(), trpcUtils.board.audit.reset(), trpcUtils.board.export.reset(), trpcUtils.board.notificationPreference.reset(), trpcUtils.board.comments.reset()]); setLoggingOut(false); } catch { toast.error("Çıxış tamamlanmadı. Yenidən cəhd edin."); } }
  ...
  return <div className="min-h-screen bg-[#08121B] text-[#E9EEF2]" style={{ backgroundColor: activeTheme.surface, backgroundImage: `linear-gradient(rgba(8,18,27,.94), rgba(8,18,27,.97)), radial-gradient(circle at 85% 0%, ${activeTheme.glow}, transparent 34%), url('/manus-storage/proline-grid_53b97e55.png')`, backgroundSize: "cover", backgroundAttachment: "fixed", "--proline-accent": activeTheme.accent, "--proline-accent-hover": activeTheme.accentHover, "--proline-glow": activeTheme.glow, "--proline-surface": activeTheme.surface } as React.CSSProperties}>
    <header className="min-h-[76px] border-b border-white/[.08] bg-[#0B1823]/90 backdrop-blur-xl flex items-center justify-between gap-3 px-3 sm:px-5 lg:px-9 py-3 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2">{user?.role === "admin" && <button onClick={() => setSettingsOpen(true)} className="inline-flex items-center justify-center rounded-lg border border-white/10 p-2.5 text-[#B8C4CB] hover:bg-white/5 transition" title="Settings"><Settings size={17} /></button>}<button onClick={() => setPushSettingsOpen(true)} className="inline-flex items-center justify-center rounded-lg border border-white/10 p-2.5 text-[#B8C4CB] hover:bg-white/5 transition" title="Bağlı cihaz bildirişləri"><Bell size={17} /></button><button onClick={toggleSound} className={`inline-flex items-center justify-center rounded-lg border p-2.5 transition ${soundEnabled ? "border-[#7FA98B]/30 bg-[#7FA98B]/10 text-[#A8D2B0]" : "border-white/10 bg-white/[.03] text-[#8292A0]"}`} title={`Bildiriş səsi: ${soundEnabled ? "Aktiv" : "Deaktiv"}`}>{soundEnabled ? <Volume2 size={17} /> : <VolumeX size={17} />}</button><div className="hidden sm:flex items-center gap-2 rounded-full border border-white/10 bg-white/[.035] pl-2 pr-3 py-1.5"><div className="h-7 w-7 rounded-full bg-[#D78A4A] proline-theme-button text-[#1C1712] flex items-center justify-center text-xs font-bold">{user.email[0].toUpperCase()}</div><div className="leading-tight"><div className="text-xs font-semibold text-[#DDE5EA]">{user.label}</div><div className="text-[10px] text-[#8292A0]">{user.email}</div></div></div><button onClick={logout} className="p-2 rounded-lg text-[#8292A0] hover:bg-white/10 hover:text-white transition" title="Çıxış"><LogOut size={17} /></button></div>
    </header>
    ...
    {selected && !creating && <OrderModal order={selected} user={user} auditLogs={orderAuditQuery.data || []} comments={commentsQuery.data || []} commentBody={commentBody} setCommentBody={setCommentBody} onSubmitComment={submitComment} onClose={() => { setSelected(null); setCommentBody(""); }} onEdit={() => openEdit(selected)} onDelete={user?.role === "admin" ? async () => { if (!window.confirm("Bu sifariş və onun tarixçəsi silinsin?")) return; try { await deleteMutation.mutateAsync({ orderId: Number(selected.id) }); setSelected(null); await boardQuery.refetch(); toast.success("Sifariş silindi."); } catch { toast.error("Sifariş silinə bilmədi."); } } : undefined} />}
    ...
    {settingsOpen && user?.role === "admin" && <SettingsPanel templateId={excelTemplate} setTemplateId={setExcelTemplate} themeId={themeId} setThemeId={setThemeId} columnLayouts={columnLayouts} setColumnLayouts={setColumnLayouts} onExport={(format, fields) => { setSettingsOpen(false); void downloadExport(format, fields); }} onClose={() => setSettingsOpen(false)} />}
